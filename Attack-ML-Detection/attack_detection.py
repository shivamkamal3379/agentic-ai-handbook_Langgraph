...
# NEW: Import Live Training
try:
    from live_training import LiveTrainingPipeline

    print("✅ Live Training Module Loaded")
except ImportError as e:
    print(f"❌ Failed to load Live Training Module: {e}")
    LiveTrainingPipeline = None
...
# NEW: Initialize Live Training Pipeline
# We start with threshold=10 just for testing/demo purposes
if LiveTrainingPipeline:
    training_pipeline = LiveTrainingPipeline(retrain_threshold=10)
    # Optional: Initialize with CSV if providing one later
    # training_pipeline.initialize_from_csv("path/to/data.csv")
else:
...
# Global store for recent features to allow training on unblock
# Key: IP, Value: Features (list)
recent_features_cache = {}
...
def preprocess_data(filepath):
    try:
        ...
        # Cache features for live training (IP -> feature vector)
        for ip, feats in zip(
            df_display["Src IP"].tolist(), df_features.values.tolist()
        ):
            recent_features_cache[ip] = feats
...
app.layout = html.Div(
    [
        html.H1(
            "Snort Live Dashboard (WOA-XGBoost and Isolation Forest)",
...
def update_dashboard(n):
    ...
    accuracy_fig = px.bar(
        x=["Current Accuracy", "Max Accuracy"],
        y=[accuracy, max_accuracy],
        labels={"x": "Metric", "y": "Accuracy"},
        title="Current vs Max Accuracy",
    )

    for ip, pred, proba in zip(df_display["Src IP"], xgb_preds, xgb_preds_proba):
        if pred == 1 and proba > 0.8 and ip not in blocked_ips:
            if block_ip(ip):
                alert_msg = f"🚨 **Attack Detected & Blocked!**\n\nIP: {ip}\nConfidence: {proba:.2f}\nAction: Blocked via iptables"
...
def unblock_ip_callback(active_cell, table_data):
    if active_cell and active_cell["column_id"] == "Unblock":
        ...
        if unblock_ip(ip_to_unblock):
            # Add to training as "Normal" (0)
            if training_pipeline and ip_to_unblock in recent_features_cache:
                print(f"🧠 Learning from mistake: {ip_to_unblock} is NORMAL")
                feats = recent_features_cache[ip_to_unblock]
                training_pipeline.append_correction(feats, label=0)
            elif training_pipeline:
                print(
                    f"⚠️ Could not find features for {ip_to_unblock}, skipping training."
                )
